...
// =====================================================
// Simplified Test Data Insertion Script
// =====================================================
 ...
 * 插入测试用户 (使用原生SQL)
 */
async function insertTestUsers(): Promise<any[]> {
  console.log('🔄 插入测试用户...');
  ...
  for (const userData of TEST_USERS) {
    try {
      ...
      await sequelize.query(
        ...
        {
          replacements: [userData.username, userData.email, hashedPassword, now, now],
 ...
 * 插入测试分类 (使用原生SQL)
 */
async function insertTestCategories(users: any[]): Promise<any[]> {
  console.log('🔄 插入测试分类...');
  ...
  for (const categoryData of TEST_CATEGORIES) {
    try {
      ...
      const [existingCategories] = await sequelize.query(
        'SELECT id, name FROM categories WHERE name = ? AND scopeType = ?',
        {
          replacements: [categoryData.name, 'public'],
          type: QueryTypes.SELECT,
        }
      );
      ...
      await sequelize.query(
        'INSERT INTO categories (name, description, scopeType, createdBy, color, isActive, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        {
          replacements: [
            categoryData.name,
 ...
 * 插入测试提示词 (使用原生SQL)
 */
async function insertTestPrompts(users: any[], categories: any[]): Promise<void> {
  console.log('🔄 插入测试提示词...');
  ...
  for (let i = 0; i < promptsToCreate; i++) {
    try {
      ...
      // 决定是否公开（80%概率公开）
      const isPublic = Math.random() < 0.8 ? 1 : 0;
      
      const promptTitle = template.title + titleVariation;
      
      // 检查是否已存在相同标题的提示词
      const [existingPrompts] = await sequelize.query(
        'SELECT id FROM prompts WHERE title = ? AND userId = ?',
        {
          replacements: [promptTitle, user.id],
          type: QueryTypes.SELECT,
        }
      );
      
      if (existingPrompts) {
        console.log(`   ⚠️  提示词 "${promptTitle}" 已存在，跳过`);
        continue;
      }
      
      // 创建提示词
      const now = new Date().toISOString();
      const tagsJson = template.tags ? JSON.stringify(template.tags) : null;
      
      await sequelize.query(
        'INSERT INTO prompts (title, content, description, version, categoryId, tags, userId, isPublic, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        {
          replacements: [
            promptTitle,
            template.content,
      ...
      createdCount++;
      console.log(`   ✅ 创建提示词 ${createdCount}: "${promptTitle}" (作者: ${user.username}, 分类: ${category.name})`);
